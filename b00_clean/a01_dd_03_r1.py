...
fd_out='./out/a01_dd_03_r1'
fd_in='./out/a01_dd_01_r0'
...
#-----------------------------------------------------
class seq_dense: 
    def __init__(self, n_layer=1, lr=0.0001, r=0):
        self.layer=n_layer
        self.lr=lr
        self.r=r
...
for fname in l_fname:
    ...
    ada=sc.read(fname)
    df=ada.to_df()  #all cells (include low reliable)
    df_hr=ada[ada.obs['dd_score']<t, :].to_df() #high reliable cells
    ...
    #add to ada
    ada.obs=ada.obs.drop('dd_score', axis=1)
