...
fd_out='./out/c00_anno_00_train'
f_in='./out/b01_r1_03_update/data.h5ad'
f_gene='./out/b00_r0_00_mark/gm_filtered.csv'

######################################################
Path(fd_out).mkdir(exist_ok=True, parents=True)

#gene
dfg=pd.read_csv(f_gene, index_col=0)
l_gene=dfg.index.tolist()

#ada
ada=sc.read(f_in)
ada.obs=ada.obs.drop('predict', axis=1)
ada.obs['prelabel']=ada.obs['prelabel'].astype('str')
ada.obs.loc[ada.obs['prelabel'].isin(l_remove), ['prelabel']]='TBD'
ada_save=ada.copy()
...
def plt_dr(ada, f_out, title=None, cmap='tab20', sz=(7.5, 6), s=3.5, obsm='X_harmony', col='predict'):
    #prep
    ...
    plt.legend(loc=(1.0, 0.15), ncol=1, frameon=False, prop={'size': 10, 'weight': 'semibold'})
    #save
    plt.tight_layout()
